...
function Login() {
  ...
  const handleLogin = async () => {
    ...
    if (role === 'admin') {
      ...
      if (voterID === 'test123' && password === 'password') {
        localStorage.setItem('token', 'mock-token-123');
        localStorage.setItem('voter', JSON.stringify({ name: 'Tooba Zainab', voterID, avatarId: 'v' }));
        navigate('/dashboard');
      } else {
        setError('Invalid Voter ID or password. Try test123 / password');
      }
    }
    setLoading(false);
  };
        ...
        <div className="auth-card fade-in">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '1.5rem' }}>
            {[
              ...
              <button key={r.id}
                ...
                <div style={{ color: role === r.id ? 'var(--violet2)' : 'var(--text2)' }}>
                  <r.Icon />
                </div>
                <div style={{ fontFamily: 'var(--font-head)', fontSize: '0.88rem', fontWeight: 700, color: role === r.id ? 'var(--violet2)' : 'var(--text2)' }}>
                  {r.label}
                </div>
          ...
          <button className="btn btn-primary btn-full" onClick={handleLogin} disabled={loading}
            ...
            {loading
              ...
              : <><div className="btn-shimmer" />{role === 'admin' ? 'Sign In as Admin' : 'Sign In to BlockVote'}</>}
          </button>
          ...
          <div style={{ marginTop: '1.5rem', padding: '12px', background: 'rgba(124,92,252,0.06)', border: '1px solid rgba(124,92,252,0.15)', borderRadius: 'var(--r-sm)', fontSize: '0.78rem', color: 'var(--text3)', textAlign: 'center', lineHeight: 1.7 }}>
            {role === 'voter'
              ? <>Voter demo: <strong style={{ color: 'var(--violet2)' }}>test123</strong> / <strong style={{ color: 'var(--violet2)' }}>password</strong></>
              : <>Admin demo: <strong style={{ color: 'var(--violet2)' }}>admin</strong> / <strong style={{ color: 'var(--violet2)' }}>admin123</strong></>}
          </div>
