    ...
    const fetchData = async () => {
      try {
        ...
        if (data.success) {
          ...
          const mapped = data.candidates.map((c, i) => ({
            _id: c._id,
            contractId: i + 1,
            name: c.name,
            party: c.party,
            manifesto: c.manifesto || 'No manifesto provided.',
            avatarCls: c.avatarCls || 'avatar-v',
  ...
  const submitVote = async () => {
    ...
    try {
      if (window.ethereum) {
        ...
        // Save to backend with real txHash
        ...
        setTimeout(() => {
          navigate('/receipt', { state: { txHash: tx.hash, blockNumber: receipt.blockNumber, candidate: selectedCandidate, electionId } });
        }, 1000);
        return;
      }
      ...
      console.log('Using backend/mock transaction:', err.message);

      // Fallback — save to backend without blockchain hash
      try {
        ...
        setTimeout(() => {
          navigate('/receipt', { state: { txHash: mockHash, blockNumber: 0, candidate: selectedCandidate, electionId } });
        }, chainSteps.length * 900 + 500);
      } catch (backendErr) {
        setError(backendErr.message || 'Vote failed. Please try again.');
        setStep(1);
      }
    }
  };
  ...
  if (loading) return (
    <div style={{ position: 'relative' }}>
      ...
      <div className="page"><div className="wrap-sm" style={{ textAlign: 'center', paddingTop: '4rem' }}>
        ...
        <p style={{ color: 'var(--text3)' }}>Loading candidates...</p>
      </div></div>
    </div>
  );
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div className="page">
        <div className="wrap-sm">
          ...
          {electionTitle && (
            <div style={{ fontSize: '0.75rem', color: 'var(--violet2)', fontWeight: 600, marginBottom: '0.5rem', letterSpacing: '0.5px' }}>
              {electionTitle}
            </div>
          )}
          ...
          {step === 0 && (
            <div className="fade-in">
              ...
              {candidates.length === 0 ? (
                <div className="alert alert-info">No candidates have been added to this election yet.</div>
              ) : (
            ...
            <div className="fade-in">
              ...
              <div className="confirm-box" style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <div className={`avatar avatar-lg ${selectedCandidate.avatarCls}`}>{getInitials(selectedCandidate.name)}</div>
                  <div>
                    <div style={{ fontFamily: 'var(--font-head)', fontSize: '1.15rem', fontWeight: 700, marginBottom: '4px' }}>{selectedCandidate.name}</div>
                    <div style={{ color: selectedCandidate.color, fontSize: '0.9rem', fontWeight: 500 }}>{selectedCandidate.party}</div>
                  </div>
                </div>
